...
export function OCCHeader({ user }: { user: any }) {
  ...
  return (
    <motion.header 
      ...
      className="flex shrink-0 items-center justify-between px-4 sm:px-8 lg:px-8 py-4 sm:py-6 bg-white/80 backdrop-blur-3xl z-40 w-full border-b border-black/[0.03] sticky top-0 shadow-sm"
    >
      {/* Premium Search Section */}
      <div className="flex flex-1 items-center max-w-xl mr-2 sm:mr-0">
        <div className={`relative w-full transition-all duration-500 ease-out ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
          ...
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Search..."
              className="h-10 sm:h-14 w-full rounded-2xl bg-black/[0.03] border border-black/5 px-10 sm:px-15 text-[13px] sm:text-[15px] font-bold text-black outline-none placeholder:text-black/30 focus:bg-white focus:shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all duration-500"
            />
            <div className="absolute right-5 hidden sm:flex items-center gap-2 pointer-events-none">
              <kbd className="inline-flex h-6 items-center gap-1 rounded-lg border border-black/10 bg-white px-2 font-mono text-[11px] font-black text-black/30 shadow-sm">
        ...
        >
          <div className="hidden sm:flex flex-col items-end leading-none">
            <div className="flex items-center gap-1.5">
              <span className="text-[14px] sm:text-[15px] font-black text-black tracking-tight">{user.fullName || "User"}</span>
              <Sparkles className="h-3 w-3 text-[#D4AF37]" fill="currentColor" />
            </div>
            <span className="text-[9px] sm:text-[10px] font-black text-[#D4AF37] uppercase tracking-[0.2em] mt-0.5">Elite Member</span>
          </div>
